...
///  * [ScrollPositionWithSingleContext], the main implementation of this
///    interface.
      ...
      if (_offsetSinceLastStop == null) {
        ...
        if (_offsetSinceLastStop.abs() > motionStartDistanceThreshold) {
          ...
          if (offset.abs() > _bigThresholdBreakDistance) {
            ...
            return math.min(
                    // Ease into the motion when the threshold is initially
                    // broken to avoid a visible jump.
...
class DragScrollActivity extends ScrollActivity {
  ...
  void dispatchScrollEndNotification(
      ScrollMetrics metrics, BuildContext context) {
    // We might not have DragEndDetails yet if we're being called from
    // beginActivity.
    final dynamic lastDetails = _controller.lastDetails;
...
/// finger off the screen to continue the scrolling gesture with the current
/// velocity.
///
