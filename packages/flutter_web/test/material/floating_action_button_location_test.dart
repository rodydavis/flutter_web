...
void main() {
  group('Basic floating action button locations', () {
    ...
    testWidgets('interrupts in-progress animations without jumps',
        (WidgetTester tester) async {
      ...
      void check() {
        ...
        if (previousRect != null && currentRect != null) {
          ...
          expect(deltaWidth.abs(), lessThanOrEqualTo(maxDeltaWidth),
              reason:
                  "The Floating Action Button's width should not change faster than $maxDeltaWidth per animation step.");
          expect(deltaHeight.abs(), lessThanOrEqualTo(maxDeltaHeight),
              reason:
                  "The Floating Action Button's width should not change faster than $maxDeltaHeight per animation step.");
        }
  ...
  testWidgets('Start-top floating action button location LTR',
    ...
    expect(tester.getRect(find.byType(FloatingActionButton)),
        Rect.fromLTWH(16.0, 28.0, 56.0, 56.0));
  });
  ...
  testWidgets('Start-top floating action button location RTL',
    ...
    expect(tester.getRect(find.byType(FloatingActionButton)),
        Rect.fromLTWH(800.0 - 56.0 - 16.0, 28.0, 56.0, 56.0));
  });
