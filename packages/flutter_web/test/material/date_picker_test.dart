...
void _tests() {
  ...
  testWidgets('render picker with intrinsic dimensions',
  ...
  Future<void> preparePicker(
      WidgetTester tester, Future<void> callback(Future<DateTime> date)) async {
  ...
  testWidgets('Can select a year and then a day', (WidgetTester tester) async {
    await preparePicker(tester, (Future<DateTime> date) async {
      ...
      final MaterialLocalizations localizations =
          MaterialLocalizations.of(tester.element(find.byType(DayPicker)));
      final String dayLabel =
          localizations.formatMediumDate(DateTime(2017, DateTime.january, 15));
  ...
  testWidgets('Current year is initially visible in year picker',
      (WidgetTester tester) async {
  ...
  testWidgets('Cannot select a day outside bounds',
    ...
    await preparePicker(tester, (Future<DateTime> date) async {
      await tester
          .tap(find.text('10')); // Earlier than firstDate. Should be ignored.
      await tester
          .tap(find.text('20')); // Later than lastDate. Should be ignored.
      await tester.tap(find.text('OK'));
  ...
  testWidgets('Can select initial date picker mode',
      (WidgetTester tester) async {
  ...
  group('haptic feedback', () {
    ...
    testWidgets('tap-select unselectable date does not vibrate',
        (WidgetTester tester) async {
      await preparePicker(tester, (Future<DateTime> date) async {
  ...
  testWidgets('chervons animate when scrolling month picker',
      (WidgetTester tester) async {
    final Key _datePickerKey = UniqueKey();
    DateTime _selectedDate = DateTime(2016, DateTime.july, 26);
    ...
    final List<RenderAnimatedOpacity> chevronRenderers = chevronFinder
        .evaluate()
        .map((Element element) => element.ancestorRenderObjectOfType(
            const TypeMatcher<RenderAnimatedOpacity>()))
        .cast<RenderAnimatedOpacity>()
        .toList();
