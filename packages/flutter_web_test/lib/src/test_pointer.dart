...
class TestPointer {
  ...
  /// argument.
  PointerDownEvent down(Offset newLocation,
      {Duration timeStamp = Duration.zero}) {
    assert(!isDown);
    _isDown = true;
    _location = newLocation;
    return PointerDownEvent(
        timeStamp: timeStamp, pointer: pointer, position: location);
  }
  ...
  /// can give a specific time stamp by passing the `timeStamp`
  /// argument.
  PointerMoveEvent move(Offset newLocation,
      {Duration timeStamp = Duration.zero}) {
    assert(isDown);
...
class TestGesture {
  ...
  static Future<TestGesture> down(
    Offset downLocation, {
    int pointer = 1,
  ...
  /// argument, and a function to use for dispatching events should be provided
  /// via the `dispatcher` argument.
  static Future<TestGesture> downWithCustomEvent(
    Offset downLocation,
